...
export default function ClaimCard({ card, compact, quiet, onOpen }) {
  ...
  // Host of the actual-source link (shown after the source name). Relative links (e.g. /method) show as-is.
  const actualHost = rendered.actualSourceUrl ? hostnameFromUrl(rendered.actualSourceUrl) || rendered.actualSourceUrl : "";

  // Card fields in contract order: speaker, exact claim, source URL, date said, horizon,
  // actual (or pending / None (reason)), actual source (name · link), grade LAST.
  const body = (
    <>
      ...
      <div style={css(`margin-top:8px;font-size:${metaSize};color:var(--body);line-height:1.65;display:flex;flex-wrap:wrap;gap:0 14px;`)}>
        ...
        <span style={metaWrap}><span style={faint}>Actual · </span><span title={card.actualTitle || undefined} data-reason-code={card.actualReasonCode || undefined}>{actualLabel}</span></span>
        <span style={metaWrap}>
          <span style={faint}>Actual source · </span>
          {rendered.actualSourcePending ? (
            "pending"
          ) : rendered.actualSourceNone ? (
            <span title={card.actualSourceReasonCode || undefined} data-reason-code={card.actualSourceReasonCode || undefined}>{rendered.actualSourceName}</span>
          ) : (
            <>
              {rendered.actualSourceName}
              {actualHost ? " · " : null}
              {actualHost ? (
      ...
      <div style={css("margin-top:10px;display:flex;align-items:center;gap:8px;")}>
        ...
        {card.reviewHoldReason ? (
          <span title={card.reviewHoldReason.title} data-reason-code={card.reviewHoldReason.code} style={css("font-size:" + metaSize + ";color:var(--muted);")}>
            {card.reviewHoldReason.label}
          </span>
        ) : null}
      </div>
    </>
