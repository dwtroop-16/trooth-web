...
import { toPublicClaimCard, actualLookup } from "./viewModel.js";
...
test("every live forecast renders a complete public card", () => {
  ...
  for (const f of FORECASTS) {
    ...
    const card = toPublicClaimCard(f, speakerBy[f.speaker_id], scoreBy[f.id], actualFor(f, scoreBy[f.id]));
...
test("hit/miss rows have resolved actuals when scores say so", () => {
  ...
  for (const s of SCORES) {
    ...
    const actual = actualFor({ match_key: s.match_key }, s);
